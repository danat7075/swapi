...
class MsgError:
    ...
    def wrong_method(self, method_name, class_name):
        return f'Проверьте правильность работы метода `{method_name}()` у класса `{class_name}`'  # noqa

    def dont_create_method(self, method_name, class_name):
        return f'`Метод `{method_name}()` не должен быть создан для класса `{class_name}`'  # noqa
    ...
    def wrong_attr(self, attr_name, class_name, msg=''):
        return f'Неверное значение свойства `{attr_name}` у класса `{class_name}`{msg}'  # noqa

    def dont_create_attr(self, attr_name, class_name):
        return f'`Свойство `{attr_name}` не должен быть создано для класса `{class_name}`'  # noqa
